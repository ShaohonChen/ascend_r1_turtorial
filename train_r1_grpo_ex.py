...
def multiplication_or_division_rewards(completions, target, nums, **kwargs):
    """
    针对乘除法的使用进行额外的奖励
    ...
    for completion, gt, numbers in zip(completions, target, nums):
        try:
            ...
            if not re.match(allowed_pattern, equation):
                rewards.append(0.0)  # 如果方程包含不允许的字符，奖励为 0
                continue
            if '*' in equation or '/' in equation:  # 鼓励模型使用乘法和除法
                # 计算方程的结果
                result = eval(equation, {"__builtins__": None}, {})
                # 检查方程是否正确且与预期答案匹配（误差小于 1e-5）
                if abs(float(result) - float(gt)) < 1e-5:
                    rewards.append(0.5)  # 正确结果且使用乘除法增加奖励
                else:
                    rewards.append(0.01)  # 如果不正确，采用乘除法仍将获得鼓励
            else:
                rewards.append(0.0) # 未使用乘除法不得分
        except Exception:
            rewards.append(0.0)  # 如果评估失败，奖励为 0
...
def grpo_function(
    ...
    trainer = GRPOTrainer(
        ...
        reward_funcs=[
            ...
            multiplication_or_division_rewards, # 额外的乘除法奖励
        ],
        args=training_args,
